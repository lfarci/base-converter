...
- Every accent-ink boundary uses **one ratio: 55% accent, 45% ink** (`.digit-box`'s
  `--digit-frame`, the highlighted place-value label's border, the breakdown term's border,
  and the highlight ring all use it). This keeps each cue tied to the base accent while
  avoiding the raw accent's insufficient contrast against its tinted surface. If you add
  another accent-ink boundary, use 55/45 and measure it against both adjacent surfaces.
