...
describe('displayedRows', () => {
  ...
  it('renders the selected 32-bit width across every base', () => {
    const displayed = displayedRows('decimal', '0', 0n, 32)

    expect(displayed.map(({ boxes }) => boxes.length)).toEqual([10, 32, 11, 8])
  })
