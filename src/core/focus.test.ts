...
describe('row anchors', () => {
  it('names a row units cell by its last registered index', () => {
    ...
    expect(unitsCellKey(binary, 8)).toBe('binary:7')
    expect(unitsCellKey(hexadecimal, 8)).toBe('hexadecimal:1')
    expect(unitsCellKey(binary, 32)).toBe('binary:31')
    expect(unitsCellKey(hexadecimal, 32)).toBe('hexadecimal:7')
  })
...
describe('tabFromToggle', () => {
  ...
  it('skips a closed breakdown for the next row units cell', () => {
    ...
    expect(tabFromToggle(decimal, false, false, 32)).toEqual({ kind: 'cell', cellKey: 'binary:31' })
  })

  it('continues forward to the footer after the last row', () => {
    expect(tabFromToggle(hexadecimal, false, false)).toEqual({ kind: 'footer-link' })
  })
})
...
describe('tabFromBreakdownTerm', () => {
  ...
  it('continues forward to the footer after the last row breakdown', () => {
    expect(tabFromBreakdownTerm(hexadecimal, false, false, true)).toEqual({ kind: 'footer-link' })
  })
})
