...
test('width slider snaps with keyboard and pointer, and breakdowns toggle in bulk', async ({ page }) => {
  ...
  await expect(readout.locator('xpath=..')).toHaveCSS('column-gap', '4px')
  ...
  expect(thumbBounds.y - channelBounds.y).toBeGreaterThanOrEqual(1)
  expect(channelBounds.y + channelBounds.height - (thumbBounds.y + thumbBounds.height)).toBeGreaterThanOrEqual(1)
  expect(valueLabelBounds.y - (thumbBounds.y + thumbBounds.height)).toBeGreaterThanOrEqual(8)
  ...
  const widthLabelBounds = await page.locator('#bit-width-label').boundingBox()
  ...
  expect(widthLabelBounds).not.toBeNull()
  expect(controlRowBounds).not.toBeNull()
  if (!guidanceBounds || !buttonBounds || !widthControlBounds || !widthLabelBounds || !controlRowBounds) return
  expect(Math.abs((buttonBounds.y + buttonBounds.height / 2) - (widthControlBounds.y + widthControlBounds.height / 2))).toBeLessThanOrEqual(1)
  expect(widthLabelBounds.x).toBeLessThan(widthControlBounds.x)
  expect(widthControlBounds.x + widthControlBounds.width).toBeLessThan(buttonBounds.x)
  expect(Math.abs(controlRowBounds.x + controlRowBounds.width - 12 - buttonBounds.x - buttonBounds.width)).toBeLessThanOrEqual(1)
  expect(Math.abs(guidanceBounds.x - widthLabelBounds.x)).toBeLessThanOrEqual(1)
  await expect(bulkButton).toHaveCSS('min-height', '44px')
  ...
  const mobileLabelBounds = await page.locator('#bit-width-label').boundingBox()
  ...
  if (!mobileButtonBounds || !mobileGuidanceBounds || !mobileSliderBounds || !mobileLabelBounds || !mobileControlRowBounds) return
  expect(Math.abs((mobileButtonBounds.y + mobileButtonBounds.height / 2) - (mobileSliderBounds.y + mobileSliderBounds.height / 2))).toBeLessThanOrEqual(1)
  expect(Math.abs(mobileLabelBounds.x - mobileSliderBounds.x)).toBeLessThanOrEqual(1)
  expect(mobileSliderBounds.x + mobileSliderBounds.width).toBeLessThan(mobileButtonBounds.x)
  ...
  expect(Math.abs(mobileGuidanceBounds.x - mobileLabelBounds.x)).toBeLessThanOrEqual(1)
