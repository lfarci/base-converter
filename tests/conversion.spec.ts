...
test('width slider snaps with keyboard and pointer, and breakdowns toggle in bulk', async ({ page }) => {
  ...
  await expect(readout).toHaveCSS('min-height', '40px')
  ...
  await expect(thumb.locator('span')).toHaveCSS('height', '28px')
  const channel = slider.locator('[data-slider-channel="true"]')
  await expect(channel).toHaveCSS('height', '40px')
  ...
  if (!guidanceBounds || !buttonBounds || !widthControlBounds || !controlGroupBounds || !channelBounds || !readoutBounds || !bulkButtonBounds || !controlRowBounds) return
  ...
  await expect(bulkButton).toHaveCSS('min-height', '40px')
  ...
  if (!mobileButtonBounds || !mobileGuidanceBounds || !mobileSliderBounds || !mobileControlGroupBounds || !mobileChannelBounds || !mobileReadoutBounds || !mobileControlRowBounds) return
